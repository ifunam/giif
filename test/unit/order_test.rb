...
class OrderTest < ActiveSupport::TestCase
 ...
 #                                     {:id => @existent_product.id, :quantity => @existent_product.quantity,  :price_per_unit => @existent_product.price_per_unit, 
 #                                       :description => 'Routers marca B', :product_category_id => 1 }]
 #     @order.valid?
 #     puts @order.errors.full_messages
 # #    assert_equal 3, @order.products.count
 #   end
