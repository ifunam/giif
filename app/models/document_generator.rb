...
class DocumentGenerator
  ...
  def to_pdf
    ...
    #user data
    pdf.add_text(478, 692, "", 10)                                                      #folio???
    pdf.add_text(468, 671, date_string, 10)                                       #date
    pdf.add_text(165, 672, to_iso("Física Teórica"), 10)                                #adscription
    pdf.add_text(165, 647, to_iso("Fernando Reyes Jiménez"), 10)               #name
    pdf.add_text(165, 628, "26 21 20 44", 10)                                    #telephone
    ...
    @data.order_products.each do |product|
      ...
      pdf.add_text(477, 608 - 19*i, "import", 10)                                     #import???
    end
    pdf.add_text(477, 404, @data.calculate_total, 10)                             #total????
    ...
    #project files
    @data.order_files.each do |file|
      pdf.add_text(get_x(file.file_type_id), 322, "X", 10)                                   #type
    end

    #project type
    @data.projects.each do |project|
      pdf.add_text(get_x(project.project_type_id), get_y(project.project_type_id), "X", 10)           #type
      pdf.add_text(185, 269, to_iso(project.name), 10)                        #name
      pdf.add_text(471, 255, project.key, 10)                                       #key
    end
